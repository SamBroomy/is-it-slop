...
push to main
  └─ Crates.io Publish ── publish crates to crates.io
       ...
       └─ Release Assets ── CLI binaries + Android AAR → GitHub Release
```
...
4. **GitHub Release created** → triggers both `PyPI Publish` and `Release Assets` (parallel)
5. **Result**: new version on crates.io, PyPI, and GitHub Releases

## Manual fallback

Both `PyPI Publish` and `Release Assets` accept a `workflow_dispatch` with a `tag` input — useful if a `release: published` event was missed (e.g. PAT cascading failure).

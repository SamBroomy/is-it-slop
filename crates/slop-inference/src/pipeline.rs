...
pub fn predict(session: &Mutex<Session>, input: &str) -> ort::Result<(Vec<i64>, Vec<Vec<f32>>)> {
...
fn run_inference(
...
) -> ort::Result<(Vec<i64>, Vec<Vec<f32>>)> {
    let input_name = session.inputs[0].name.clone();
    ...
    let probs_vec = probs.outer_iter().map(|row| row.to_vec()).collect();

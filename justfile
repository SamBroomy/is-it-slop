...
build-cli-release:
    cargo build --release --bin is-it-slop
...
run-cli:
    ...
    cargo run -q --release --bin is-it-slop -- "This is a test text to check if it's AI generated."
    @echo ""
    @echo "2. Class format (just 0 or 1):"
    cargo run -q --release --bin is-it-slop -- "This is a test text." --format class
    @echo ""
    @echo "3. JSON format (detailed output):"
    cargo run -q --release --bin is-it-slop -- "This is a test text." --format json
    @echo ""
    @echo "4. Human-readable format:"
    cargo run -q --release --bin is-it-slop -- "This is a test text." --format human
    @echo ""
    @echo "5. Verbose mode (shows timing):"
    cargo run -q --release --bin is-it-slop -- "This is a test." --verbose
    @echo ""
    @echo "6. Custom labels with JSON:"
    cargo run -q --release --bin is-it-slop -- "This is a test." --labels real fake --format json
    @echo ""
    @echo "7. Quiet mode with class output:"
    cargo run -q --release --bin is-it-slop -- "This is a test." --quiet --format class
    @echo ""
...
test-cli TEXT:
    cargo run -q --release --bin is-it-slop -- "{{TEXT}}"

# Show CLI help
cli-help:
    cargo run -q --release --bin is-it-slop -- --help
